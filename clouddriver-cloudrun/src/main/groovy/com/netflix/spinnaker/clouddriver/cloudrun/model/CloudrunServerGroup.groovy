...
class CloudrunServerGroup implements ServerGroup, Serializable {
  String name
  ...
  CloudrunScalingPolicy scalingPolicy
  ...
  CloudrunServerGroup(Version version, String account, String region, String loadBalancerName, Boolean isDisabled) {
    this.account = account
    this.region = region
    this.name = version.getId()
    this.loadBalancers = [loadBalancerName] as Set
    this.createdTime = CloudrunModelUtil.translateTime(version.getCreateTime())
    this.disabled = isDisabled
    this.scalingPolicy = CloudrunModelUtil.getScalingPolicy(version)
    ...
    this.httpUrl = CloudrunModelUtil.getHttpUrl(version.getName())
    this.httpsUrl = CloudrunModelUtil.getHttpsUrl(version.getName())
  ...
  Integer computeMinForAutomaticScaling(CloudrunScalingPolicy scalingPolicy) {
    Integer instanceCount = instances?.size() ?: 0
  ...
  Integer computeMaxForAutomaticScaling(CloudrunScalingPolicy scalingPolicy) {
    Integer instanceCount = instances?.size() ?: 0
    if (servingStatus == ServingStatus.SERVING) {
  ...
  static Boolean versionAllowsGradualTrafficMigration(Version version) {
    ...
    def usesAutomaticScaling = CloudrunModelUtil.getScalingPolicy(version).type == ScalingPolicyType.AUTOMATIC
