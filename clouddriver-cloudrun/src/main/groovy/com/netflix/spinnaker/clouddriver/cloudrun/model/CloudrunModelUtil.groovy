...
import com.netflix.spinnaker.clouddriver.cloudrun.deploy.description.UpsertCloudrunLoadBalancerDescription.CloudrunTrafficSplitDescription

import java.text.SimpleDateFormat

class CloudrunModelUtil {
  ...
  static CloudrunScalingPolicy getScalingPolicy(Version version) {
    if (version.getAutomaticScaling()) {
      return new CloudrunScalingPolicy(version.getAutomaticScaling())
    } else if (version.getBasicScaling()) {
      return new CloudrunScalingPolicy(version.getBasicScaling())
    } else if (version.getManualScaling()) {
      return new CloudrunScalingPolicy(version.getManualScaling())
    } else {
      return new CloudrunScalingPolicy()
    }
  }

  static CloudrunTrafficSplit convertTrafficSplitDescriptionToTrafficSplit(CloudrunTrafficSplitDescription splitDescription) {
    ...
    return new CloudrunTrafficSplit(
