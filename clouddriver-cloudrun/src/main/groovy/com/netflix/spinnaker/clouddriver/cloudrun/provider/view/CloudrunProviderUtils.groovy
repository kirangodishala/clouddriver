...
@Slf4j
class CloudrunProviderUtils {
  static CloudrunServerGroup serverGroupFromCacheData(ObjectMapper objectMapper,
                                                       CacheData cacheData,
                                                       Set<CloudrunInstance> instances) {
    CloudrunServerGroup serverGroup = objectMapper.convertValue(cacheData.attributes.serverGroup, CloudrunServerGroup)
    serverGroup.instances = instances
    serverGroup
  }

  static CloudrunInstance instanceFromCacheData(ObjectMapper objectMapper, CacheData instanceData) {
    if (!instanceData) {
      return null
    } else {
      def instance = objectMapper.convertValue(instanceData.attributes.instance, CloudrunInstance)
  ...
  static CloudrunLoadBalancer loadBalancerFromCacheData(ObjectMapper objectMapper,
                                                         CacheData loadBalancerData,
                                                         Set<CloudrunServerGroup> serverGroups) {
    def loadBalancer = objectMapper.convertValue(loadBalancerData.attributes.loadBalancer, CloudrunLoadBalancer)
