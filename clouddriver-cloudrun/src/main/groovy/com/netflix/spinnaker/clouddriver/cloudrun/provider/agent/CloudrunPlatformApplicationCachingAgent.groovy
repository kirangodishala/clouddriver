...
import com.netflix.spinnaker.clouddriver.cloudrun.model.CloudrunPlatformApplication
...
@Slf4j
class CloudrunPlatformApplicationCachingAgent extends AbstractCloudrunCachingAgent {
  String agentType = "${accountName}/${CloudrunPlatformApplicationCachingAgent.simpleName}"

  static final Set<AgentDataType> types = Collections.unmodifiableSet([
    AUTHORITATIVE.forType(Keys.Namespace.PLATFORM_APPLICATIONS.ns)] as Set
  )

  CloudrunPlatformApplicationCachingAgent(String accountName,
  ...
  String getSimpleName() {
    CloudrunPlatformApplicationCachingAgent.simpleName
  }
  ...
  CacheResult loadData(ProviderCache providerCache) {
    ...
    cachedPlatformApplications[apiApplicationKey].with {
      ...
      attributes.platformApplication = new CloudrunPlatformApplication(platformApplication)
    }
