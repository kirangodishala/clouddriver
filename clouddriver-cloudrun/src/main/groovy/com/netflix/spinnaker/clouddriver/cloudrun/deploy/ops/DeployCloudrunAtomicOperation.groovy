...
import com.netflix.spinnaker.clouddriver.cloudrun.artifacts.GcsStorageService
import com.netflix.spinnaker.clouddriver.cloudrun.artifacts.config.StorageConfigurationProperties
import com.netflix.spinnaker.clouddriver.cloudrun.deploy.CloudrunMutexRepository
...
import com.netflix.spinnaker.clouddriver.cloudrun.gcsClient.CloudrunGcsRepositoryClient
...
class DeployCloudrunAtomicOperation implements AtomicOperation<DeploymentResult> {
  ...
  DeploymentResult operate(List priorOutputs) {
    ...
    */
    registry.counter(registry.createId("appengine.deployStart",
                                       ...
                                       "region", region))
            .increment()
    return CloudrunMutexRepository.atomicWrapper(directoryPath, {
      ...
      try {
          ...
          def duration = registry.clock().monotonicTime() - startTime
          registry.timer(registry.createId("cloudrun.deploy",
                                           "success", success,
                                           "account", serviceAccount,
                                           "region", region))
              .record(duration, TimeUnit.NANOSECONDS);
  ...
  void createEmptyDirectory(String path) {
    ...
    if (!directory.mkdirs()) {
      throw new CloudrunOperationException("Failed to create directory: $path")
    }
  }
  ...
  String cloneOrUpdateLocalRepository(String directoryPath, Integer retryCount) {
    ...
    if (usesGcs) {
      ...
      repositoryClient = new CloudrunGcsRepositoryClient(repositoryUrl, directoryPath, applicationDirectoryRoot,
                                                          storage, jobExecutor)
  ...
  String deploy(String repositoryPath) {
    ...
    def serverGroupNameResolver = new CloudrunServerGroupNameResolver(project, region, description.credentials)
    ...
    try {
      ...
      def duration = registry.clock().monotonicTime() - startTime
      def id = registry.createId("cloudrun.deploy",
    ...
    if (!configArtifacts) {
      ...
      return configArtifacts.collect { artifact ->
        ...
        try {
          ...
          throw new CloudrunOperationException("Could not download artifact as config file: ${e.getMessage()}")
        }
