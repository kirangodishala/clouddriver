...
import static com.netflix.spinnaker.clouddriver.cloudrun.cache.Keys.Namespace.LOAD_BALANCERS
import static com.netflix.spinnaker.clouddriver.cloudrun.cache.Keys.Namespace.ON_DEMAND

@Slf4j
class CloudrunLoadBalancerCachingAgent extends AbstractCloudrunCachingAgent implements OnDemandAgent {
  ...
  String agentType = "${accountName}/${CloudrunLoadBalancerCachingAgent.simpleName}"

  @Override
  String getSimpleName() {
    CloudrunLoadBalancerCachingAgent.simpleName
  }
  ...
  CloudrunLoadBalancerCachingAgent(String accountName,
    ...
    metricsSupport = new OnDemandMetricsSupport(
      registry,
      this,
      "$CloudrunCloudProvider.ID:$OnDemandType.LoadBalancer")
  }

  @Override
  boolean handles(OnDemandType type, String cloudProvider) {
    type == OnDemandType.LoadBalancer && cloudProvider == CloudrunCloudProvider.ID
  }
    ...
    services.each { Service service ->
      ...
      if (onDemandData && onDemandData.attributes.cacheTime >= start) {
        ...
        cachedLoadBalancers[loadBalancerKey].with {
          ...
          attributes.loadBalancer = new CloudrunLoadBalancer(service,
                                                              accountName,
                                                              credentials.region)
        }
      }
