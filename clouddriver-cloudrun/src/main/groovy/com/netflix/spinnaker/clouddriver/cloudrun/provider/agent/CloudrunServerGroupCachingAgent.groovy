...
@Slf4j
class CloudrunServerGroupCachingAgent extends AbstractCloudrunCachingAgent implements OnDemandAgent {
  ...
  String agentType = "${accountName}/${CloudrunServerGroupCachingAgent.simpleName}"

    CloudrunServerGroupCachingAgent(String accountName,
    ...
    this.metricsSupport = new OnDemandMetricsSupport(
      registry,
      this,
      "$CloudrunCloudProvider.ID:$OnDemandType.ServerGroup")
  }

  @Override
  String getSimpleName() {
    CloudrunServerGroupCachingAgent.simpleName
  }
  ...
  boolean handles(OnDemandType type, String cloudProvider) {
    type == OnDemandType.ServerGroup && cloudProvider == CloudrunCloudProvider.ID
  }
  ...
  OnDemandAgent.OnDemandResult handle(ProviderCache providerCache, Map<String, ? extends Object> data) {
    ...
    Map<String, Collection<String>> evictions = serverGroup ? [:] : [(Keys.Namespace.SERVER_GROUPS.ns): [serverGroupKey]]
  ...
  CacheResult loadData(ProviderCache providerCache) {
    ...
    providerCache.getAll(Keys.Namespace.ON_DEMAND.ns, serverGroupKeys).each { CacheData onDemandEntry ->
      ...
      serverGroups.each { Version serverGroup ->
        ...
        if (onDemandData && onDemandData.attributes.cacheTime >= start) {
          ...
          cachedApplications[applicationKey].with {
            ...
            relationships[Keys.Namespace.LOAD_BALANCERS.ns].add(loadBalancerKey)
          }
          ...
          def instanceKeys = instances.inject([], { ArrayList keys, instance ->
            ...
            cachedInstances[key].with {
              ...
              attributes.instance = new CloudrunInstance(instance, serverGroup, loadBalancer, credentials.region)
              relationships[Keys.Namespace.APPLICATIONS.ns].add(applicationKey)
              relationships[Keys.Namespace.CLUSTERS.ns].add(clusterKey)
              relationships[Keys.Namespace.SERVER_GROUPS.ns].add(serverGroupKey)
              relationships[Keys.Namespace.LOAD_BALANCERS.ns].add(loadBalancerKey)
          ...
          cachedServerGroups[serverGroupKey].with {
            ...
            if (attributes.serverGroup == null) {
              attributes.serverGroup = new CloudrunServerGroup(serverGroup,
                accountName,
                credentials.region,
                loadBalancerName,
                isDisabled)
          ...
          cachedLoadBalancers[loadBalancerKey].with {
            ...
            attributes.loadBalancer = new CloudrunLoadBalancer(loadBalancer, accountName, credentials.region)
            relationships[Keys.Namespace.SERVER_GROUPS.ns].add(serverGroupKey)
            relationships[Keys.Namespace.INSTANCES.ns].addAll(instanceKeys)
          }
        }
  ...
  Map loadServerGroupAndLoadBalancer(String serverGroupName) {
    ...
    loadBalancers.each { Service lb ->
      ...
      def callback = new CloudrunCallback<Version>()
  ...
  Map<Version, List<Instance>> loadInstances(Map<Service, List<Version>> serverGroupsByLoadBalancer) {
    ...
    serverGroupsByLoadBalancer.each { Service loadBalancer, List<Version> serverGroups ->
      serverGroups.each { Version serverGroup ->
        def serverGroupName = serverGroup.getId()
        def callback = new CloudrunCallback<ListInstancesResponse>()
